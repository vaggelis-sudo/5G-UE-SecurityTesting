...
void test_parser(string testing_filepath){ //Parsing hereeeeeeeeeeeeeeeee

  // Open the file using the ifstream class
  ifstream input(testing_filepath);

  // Check that the file was opened successfully
  if (!input) {
    cout << "Failed to open file " << testing_filepath << endl;
    return;
  }

  // Read the entire contents of the file into a string
  string json((istreambuf_iterator<char>(input)), istreambuf_iterator<char>());
  ...
  // Check that the document is an array
  if (!doc.IsArray()) {
    cout << "JSON document is not an array" << endl;
    return;
  }
  ...
  for (SizeType i = 0; i < doc.Size(); ++i) {
    ...
    if (!object.HasMember("ue_ul_handle") || !object.HasMember("dl_reply") ||
        !object.HasMember("command_mode") || !object.HasMember("dl_params")) {
      cout << "JSON object " << i << " has missing members" << endl;
      return;
    }

    // Access the values of the members
    if ((string)object["ue_ul_handle"].GetString() != "null" && (string)object["dl_reply"].GetString() != "null" && (string)object["command_mode"].GetString() != "null"){
      
      r_command.ue_ul_handle = object["ue_ul_handle"].GetString();
      r_command.dl_reply = object["dl_reply"].GetString();
      r_command.command_mode = object["command_mode"].GetString();

      if(i == 0) cout << "Initialization for Pre-AKA" << endl;
      else if(i == 1) cout << "Initialization for AKA" << endl;
      else cout << "Initialization for Post-AKA" << endl;
      cout << "UE UL: " << r_command.ue_ul_handle << endl;
      cout << "DL Reply: " << r_command.dl_reply << endl;
      cout << "Command Mode: " << r_command.command_mode  << endl;

      if(r_command.ue_ul_handle == "rrc_setup_request") hijack_rrc_setup_request = true;
      else if(r_command.ue_ul_handle == "rrc_reestablishment_request") hijack_rrc_reestablishment_request = true;
      else if(r_command.ue_ul_handle == "rrc_setup_complete") hijack_rrc_setup_complete = true;
      else if(r_command.ue_ul_handle == "rrc_reestablishment_complete") hijack_rrc_reestablishment_complete = true;
      else if(r_command.ue_ul_handle == "rrc_reconfiguration_complete") hijack_rrc_reconfiguration_complete = true;
      else if(r_command.ue_ul_handle == "rrc_security_mode_complete") hijack_security_mode_complete = true;
      else if(r_command.ue_ul_handle == "rrc_ue_capability_information") hijack_ue_capability_information = true;
      else if(r_command.ue_ul_handle == "ul_information_transfer") hijack_ul_information_transfer = true;
      else if(r_command.ue_ul_handle == "nas_security_mode_command") hijack_security_mode_command = true;
    }else continue;

    // Check if dl_params is an object and access its values
    if (object["dl_params"].IsObject()) {
      const Value& dl_params = object["dl_params"];

      // Iterate over all the members of dl_params and output their names and values
      for (Value::ConstMemberIterator iter = dl_params.MemberBegin(); iter != dl_params.MemberEnd(); ++iter) {

        string parameter = iter->name.GetString();
        string value = iter->value.GetString();

        // Make sure to enable flags
        if(parameter == "security_mode_command" && value == "disabled") hijack_security_mode_command_disable = true;
        else{ // Push back the values
           r_command.dl_params.push_back(parameter);
           r_command.dl_params.push_back(value);
        }

        cout << "DL Params: " << parameter << " " << value << endl;
      }
      // Add to vector
      if(i == 0) r_testset.rrc_commands_before_aka.push_back(r_command);
      else if(i == 1) r_testset.rrc_commands_aka.push_back(r_command);
      else r_testset.rrc_commands_after_aka.push_back(r_command);
    }else if(object["dl_params"].IsString() && (string)object["dl_params"].GetString() == "null"){
      // Add to vector
      if(i == 0) r_testset.rrc_commands_before_aka.push_back(r_command);
      else if(i == 1) r_testset.rrc_commands_aka.push_back(r_command);
      else r_testset.rrc_commands_after_aka.push_back(r_command);
...
static void execute_cmd(metrics_stdout* metrics, srsenb::enb_command_interface* control, const string& cmd_line)
{
  ...
  }else if(cmd[0] == "test"){ //Testing parameter
    if (cmd.size() != 2) {
      cout << "Usage: " << cmd[0] << endl;
      return;
    }
    if (cmd[1].size() != strlen(cmd[1].c_str())) { //check for null characters
        return;
    }
    test_parser(cmd[1]);
...
int main(int argc, char* argv[])
{
  ...
  while (running) {
    ...
    if(r_testset.rrc_commands_before_aka.empty() && r_testset.rrc_commands_aka.empty() && r_testset.rrc_commands_after_aka.empty() && testing_mode == true) printf("Testing finished!");
