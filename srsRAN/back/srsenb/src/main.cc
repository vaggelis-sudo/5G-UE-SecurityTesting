...
int main(int argc, char* argv[])
{
  ...
  //Import and initialize testcases
  //until RRC Setup Complete it's pre-aka
  //until RRC Security Mode Complete is AKA
  //after RRC Security Mode Complete is After-AKA

  std::ifstream infile("/home/cp105/testcase.txt");
  ...
  if(infile && infile.is_open()){
    ...
    string line, word, first, second, third, fourth = "";
    int numWords;
    bool aka = false;
    bool pre_aka = false;
    bool after_aka = false;
    ...
    while(std::getline(infile, line)){
      ...
      //cout << "HEY2" <<endl;
        ...
        pre_aka = false;
        after_aka = false;
      }else if(line == "AfterAKA"){
      //cout << "HEY3" <<endl;
        after_aka = true;
        aka = false;
        pre_aka = false;
      }else if(line == "#region_end"){
      //cout << "HEY4" <<endl;
        if(pre_aka) r_testset.rrc_commands_before_aka.push_back(r_command);
        else if(aka) r_testset.rrc_commands_aka.push_back(r_command);
        else if(after_aka) r_testset.rrc_commands_after_aka.push_back(r_command);
      }else{
      //cout << "HEY5" <<endl;
        if(numWords == 4){
          ...
          third.erase(third.find_last_not_of("\n\r") + 1);
          fourth.erase(fourth.find_last_not_of("\n\r") + 1);
          if(first == "set" && second == "dl_params"){
            if(third == "smc" && fourth == "disabled") hijack_security_mode_command_disable = true;
            else{
             r_command.dl_params.push_back(third);
             r_command.dl_params.push_back(fourth);
            } 
          }else break;
        }else if(numWords == 3){
        //cout << "HEY6" <<endl;
          ...
          third.erase(third.find_last_not_of("\n\r") + 1);
          if(first == "set"){
            if(second == "ue_ul_handle"){
             r_command.ue_ul_handle = third;
             if(third == "rrc_setup_request") hijack_rrc_setup_request = true;
             else if(third == "rrc_reestablishment_request") hijack_rrc_reestablishment_request = true;
             else if(third == "rrc_setup_complete") hijack_rrc_setup_complete = true;
             else if(third == "rrc_reestablishment_complete") hijack_rrc_reestablishment_complete = true;
             else if(third == "rrc_reconfiguration_complete") hijack_rrc_reconfiguration_complete = true;
             else if(third == "rrc_security_mode_complete") hijack_security_mode_complete = true;
             else if(third == "rrc_ue_capability_information") hijack_ue_capability_information = true;
             else if(third == "ul_information_transfer") hijack_ul_information_transfer = true;
             else if(third == "nas_security_mode_complete") hijack_security_mode_command = true;
            }else if(second == "dl_reply") r_command.dl_reply = third;
            else if(second == "command_mode") r_command.command_mode = third;
          }else break;
        }else{ 
        //cout << "HEY7" <<endl;
         continue;
    ...
    //cout << "1: " << r_testset.rrc_commands_before_aka.size();
    //cout << "2: " << r_testset.rrc_commands_aka.size();
    //cout << "3: " << r_testset.rrc_commands_after_aka.size(); 
  
   for(auto i = r_testset.rrc_commands_before_aka.begin(); i != r_testset.rrc_commands_before_aka.end(); ++i){
    //cout << "Here1";
    cout << (*i).ue_ul_handle << endl;
    cout << (*i).dl_reply << endl;
    cout << (*i).command_mode << endl;
    //std::cout << *i.ue_ul_handle << endl;
   }
    
   for(auto i = r_testset.rrc_commands_aka.begin(); i != r_testset.rrc_commands_aka.end(); ++i){
    //cout << "Here2";
    cout << (*i).ue_ul_handle << endl;
    cout << (*i).dl_reply << endl;
    cout << (*i).command_mode << endl;
   ...
   for(auto i = r_testset.rrc_commands_after_aka.begin(); i != r_testset.rrc_commands_after_aka.end(); ++i){
    //cout << "Here3";
    cout << (*i).ue_ul_handle << endl;
    cout << (*i).dl_reply << endl;
    cout << (*i).command_mode << endl;
  ...
  infile.close();
  std::ofstream outfile("/home/cp105/testcase.txt", std::ios_base::app);
  ...
  while (running) {
    ...
    //write to the file once the rrc testing has finished. Handler will read the complete status and terminate gNodeB.
    if(r_testset.rrc_commands_before_aka.empty() && r_testset.rrc_commands_aka.empty() && r_testset.rrc_commands_after_aka.empty() && status_complete == false){
      outfile << endl << "RRC Testing Status: Complete!";
      outfile.close();
  ...
  hijack_security_mode_command_disable = false;
