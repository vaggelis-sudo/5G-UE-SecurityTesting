...
{
  /////////////////////////////////function hook
  if(hijack_rrc_setup_request) handle_hook("rrc_setup_request");
  else{
...
{
  /////////////////////////////////function hook
  if(hijack_rrc_reestablishment_request) handle_hook("rrc_reestablishment_request");
  else{
...
{
  /////////////////////////////////function hook
  if(hijack_rrc_setup_complete) handle_hook("rrc_setup_complete");
  else{
...
void rrc_nr::ue::send_security_mode_command(srsran::unique_byte_buffer_t nas_pdu)
{
    if(!hijack_security_mode_command_disable){
        if(hijack_security_mode_command) handle_hook("rrc_security_mode_command");
        else{
...
{
  /////////////////////////////////function hook
  if(hijack_security_mode_complete) handle_hook("rrc_security_mode_complete");
  else{
...
{
  /////////////////////////////////function hook
  if(hijack_ue_capability_information) handle_hook("rrc_ue_capability_information");
  else{
...
{
  /////////////////////////////////function hook
  if(hijack_rrc_reconfiguration_complete) handle_hook("rrc_reconfiguration_complete");
  else{
    update_mac(next_cell_group_cfg, true);
    radio_bearer_cfg = next_radio_bearer_cfg;
    cell_group_cfg   = next_cell_group_cfg;
...
{
  /////////////////////////////////function hook
  if(hijack_rrc_reestablishment_complete) handle_hook("rrc_reestablishment_complete");
  else{
...
{
  /////////////////////////////////function hook
  if(hijack_ul_information_transfer) handle_hook("ul_information_transfer");
  else{
...
void rrc_nr::ue::handle_hook(string ul_message){
  ...
  cout << "Hijacking in progress.." << endl;
...
void rrc_nr::ue::dl_message_handle(string dl_message, vector <string> params){
 ...
 else if(dl_message == "paging") user_send_rrc_paging(params);
}
...
void rrc_nr::ue::user_send_rrc_release(vector <string> params){
  ...
  for(std::size_t i = 0; i < params.size(); i+=2) {
    ...
    }else if(params[i] == "suspend_cfg"){
      //periodic_rnau_timer_value_e?
      //unnecessary?
    }else if(params[i] == "depriorit_req"){
      //unnecessary?
    }else if(params[i] == "late_non_crit_ext"){
      //unnecessary?
    }else if(params[i] == "non_crit_ext.wait_time"){
...
void rrc_nr::ue::user_send_rrc_reject(vector <string> params){
  ...
  for(std::size_t i = 0; i < params.size(); i+=2) {
    if(params[i] == "wait_time_present"){
      ...
      else ies.wait_time_present = false;
...
void rrc_nr::ue::user_send_rrc_security_mode_command(vector <string> params){
  ...
  for(std::size_t i = 0; i < params.size(); i+=2) {
    if(params[i] == "ext"){
      ...
      else ies.security_cfg_smc.security_algorithm_cfg.ext = false;
    }else if(params[i] == "integrity_prot_algorithm_present"){
      if(params[i+1] == "false") ies.security_cfg_smc.security_algorithm_cfg.integrity_prot_algorithm_present = false;
      else ies.security_cfg_smc.security_algorithm_cfg.integrity_prot_algorithm_present = true;
...
void rrc_nr::ue::user_send_rrc_setup(vector <string> params){
  ...
  srb1.srb_id = 1;
  compute_diff_radio_bearer_cfg(parent->cfg, radio_bearer_cfg, next_radio_bearer_cfg, setup_ies.radio_bearer_cfg);
  fill_cellgroup_with_radio_bearer_cfg(parent->cfg, rnti, *parent->bearer_mapper, setup_ies.radio_bearer_cfg, next_cell_group_cfg);
  srsran::unique_byte_buffer_t pdu = parent->pack_into_pdu(next_cell_group_cfg, __FUNCTION__);
  if (pdu == nullptr) {
    send_rrc_reject(16);
    return;
  ...
  for(std::size_t i = 0; i < params.size(); i+=2) {
    if(params[i] == "security_cfg_present"){
      if(params[i+1] == "true") setup_ies.radio_bearer_cfg.security_cfg_present = true;
      else setup_ies.radio_bearer_cfg.security_cfg_present = false;
    }else if(params[i] == "security_cfg_ext"){
      if(params[i+1] == "true") setup_ies.radio_bearer_cfg.security_cfg.ext = true;
      else setup_ies.radio_bearer_cfg.security_cfg.ext = false;
    }else if(params[i] == "security_algorithm_cfg_present"){
      if(params[i+1] == "true") setup_ies.radio_bearer_cfg.security_cfg.security_algorithm_cfg_present = true;
      else setup_ies.radio_bearer_cfg.security_cfg.security_algorithm_cfg_present = false;
    }else if(params[i] == "key_to_use_present"){
      if(params[i+1] == "true") setup_ies.radio_bearer_cfg.security_cfg.key_to_use_present = true;
      else setup_ies.radio_bearer_cfg.security_cfg.key_to_use_present = false;
    }else if(params[i] == "security_algorithm_cfg_ext"){
      if(params[i+1] == "true") setup_ies.radio_bearer_cfg.security_cfg.security_algorithm_cfg.ext = true;
      else setup_ies.radio_bearer_cfg.security_cfg.security_algorithm_cfg.ext = false;
    }else if(params[i] == "integrity_prot_algorithm_present"){
      if(params[i+1] == "true") setup_ies.radio_bearer_cfg.security_cfg.security_algorithm_cfg.integrity_prot_algorithm_present = true;
      else setup_ies.radio_bearer_cfg.security_cfg.security_algorithm_cfg.integrity_prot_algorithm_present = false;
    }else if(params[i] == "ciphering_algorithm"){
      if(params[i+1] == "nea0") setup_ies.radio_bearer_cfg.security_cfg.security_algorithm_cfg.ciphering_algorithm.value = ciphering_algorithm_opts::nea0;
      else if(params[i+1] == "nea1") setup_ies.radio_bearer_cfg.security_cfg.security_algorithm_cfg.ciphering_algorithm.value = ciphering_algorithm_opts::nea1;
      else if(params[i+1] == "nea2") setup_ies.radio_bearer_cfg.security_cfg.security_algorithm_cfg.ciphering_algorithm.value = ciphering_algorithm_opts::nea2;
      else if(params[i+1] == "nea3") setup_ies.radio_bearer_cfg.security_cfg.security_algorithm_cfg.ciphering_algorithm.value = ciphering_algorithm_opts::nea3;
    }else if(params[i] == "integrity_prot_algorithm"){
      if(params[i+1] == "nia0") setup_ies.radio_bearer_cfg.security_cfg.security_algorithm_cfg.integrity_prot_algorithm.value = integrity_prot_algorithm_opts::nia0;
      else if(params[i+1] == "nia1") setup_ies.radio_bearer_cfg.security_cfg.security_algorithm_cfg.integrity_prot_algorithm.value = integrity_prot_algorithm_opts::nia1;
      else if(params[i+1] == "nia2") setup_ies.radio_bearer_cfg.security_cfg.security_algorithm_cfg.integrity_prot_algorithm.value = integrity_prot_algorithm_opts::nia2;
      else if(params[i+1] == "nia3") setup_ies.radio_bearer_cfg.security_cfg.security_algorithm_cfg.integrity_prot_algorithm.value = integrity_prot_algorithm_opts::nia3;
    }else if(params[i] == "key_to_use"){
      if(params[i+1] == "master") setup_ies.radio_bearer_cfg.security_cfg.key_to_use.value = security_cfg_s::key_to_use_opts::master;
      else if(params[i+1] == "secondary") setup_ies.radio_bearer_cfg.security_cfg.key_to_use.value = security_cfg_s::key_to_use_opts::secondary;
      else if(params[i+1] == "nulltype") setup_ies.radio_bearer_cfg.security_cfg.key_to_use.value = security_cfg_s::key_to_use_opts::nulltype;
    }
  }
...
void rrc_nr::ue::user_send_rrc_ue_capability_enquiry(vector <string> params){
  ...
  //Set up the user values
  for(std::size_t i = 0; i < params.size(); i+=2) {
    if(params[i] == "eutra_nr") cap_rat_request.rat_type.value = rat_type_opts::eutra_nr;
    else if(params[i] == "eutra"){
    ...
    }else if(params[i] == "spare1") cap_rat_request.rat_type.value = rat_type_opts::spare1;
    else if(params[i] == "nulltype")cap_rat_request.rat_type.value = rat_type_opts::nulltype;
  }
...
void rrc_nr::ue::user_send_rrc_reestablishment(vector <string> params){
  ...
  for(std::size_t i = 0; i < params.size(); i+=2) {
    ...
    else if(params[i] == "late_non_crit_ext") cout << "not done" << endl; //unnecessary?
  }
...
void rrc_nr::ue::user_send_rrc_resume(vector <string> params){
  ...
  for(std::size_t i = 0; i < params.size(); i+=2) {
    if(params[i] == "non_crit_ext_present"){
      ...
      else ies.full_cfg_present = false;
    }else if(params[i] == "cell_group_id"){
      master_cell_group.cell_group_id = stringtoint(params[i+1]);
    }else if(params[i] == "rnti"){
      fill_cellgroup_with_radio_bearer_cfg(parent->cfg, stringtoint16(params[i+1]), *parent->bearer_mapper, ies.radio_bearer_cfg, master_cell_group);
...
void rrc_nr::ue::user_send_rrc_paging(vector <string> params){

  pcch_msg_s dl_pcch_msg;
  dl_pcch_msg.msg.set_c1().set_paging().non_crit_ext_present = false;
  paging_s& paging = dl_pcch_msg.msg.c1().paging();

  paging_record_s paging_record;

  //Set up the user values
  for(std::size_t i = 0; i < params.size(); i+=2) {
    if(params[i] == "non_crit_ext_present"){
      if(params[i+1] == "true") paging.non_crit_ext_present = true;
      else paging.non_crit_ext_present = false;
    }else if(params[i] == "ext"){
      if(params[i+1] == "true") paging_record.ext = true;
      else paging_record.ext = false;
    }else if(params[i] == "access_type_present"){
      if(params[i+1] == "true") paging_record.access_type_present = true;
      else paging_record.access_type_present = false;
    //}else if(params[i] == "sys_info_mod_present"){
      //if(params[i+1] == "true") paging.sys_info_mod_present = true;
      //else paging_record.sys_info_mod_present = false;
    //}else if(params[i] == "etws_ind_present"){
      //  if(params[i+1] == "true") paging.etws_ind_present = true;
      //  else paging_record.etws_ind_present = false;
    }else if(params[i] == "id_type"){
      //if(params[i+1] == "tmsi") paging_record.ue_id.type().value = paging_ue_id_c::types_opts::ng_minus5_g_s_tmsi;
      //else if(params[i+1] == "rnti") paging_record.ue_id.type().value = paging_ue_id_c::types_opts::full_i_rnti;
    }else if(params[i] == "tmsi"){
      //paging_record.ue_id.
    }else if(params[i] == "rnti"){
      //paging_record.ue_id.
    }else if(params[i] == "late_non_crit_ext"){
  ...
  paging.paging_record_list.push_back(paging_record);
...
uint8 stringtoint(string str){
  const char* new_str = str.c_str();
  uint8* data = new uint8[strlen(new_str) + 1];

  memcpy(data, new_str, strlen(new_str) + 1);
  return *data;
}

uint16 stringtoint16(string str){
  const char* new_str = str.c_str();
  uint16* data = new uint16[strlen(new_str) + 1];

  memcpy(data, new_str, strlen(new_str) + 1);
  return *data;
}

uint32 stringtoint32(string str){
  const char* new_str = str.c_str();
  uint32* data = new uint32[strlen(new_str) + 1];
